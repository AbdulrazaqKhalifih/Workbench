...
export default function ProjectDetailPage() {
  ...
  const { user } = useAuth();
  const { getProject, fetchProjects, loading: projectLoading } = useProjects();
  ...
  const handleEditTask = async (e) => {
    ...
    if (editTask.description !== undefined) updates.description = editTask.description;
    ...
    <div className="px-6 py-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {STATUS_COLUMNS.map((col) => {
          ...
          return (
            <div key={col.key} className="rounded-md border border-gray-200 bg-gray-50/50">
              {/* Column header */}
              <div className="flex items-center justify-between px-3 py-2 border-b border-gray-200">
                <div className="flex items-center gap-2">
                  <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${col.color}`}>
                    {col.label}
                  </span>
                  <span className="text-[10px] text-gray-400">{columnTasks.length}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh]">
          ...
          <div className="relative z-10 w-full max-w-md rounded-md border border-gray-200 bg-white shadow-lg">
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
              <h2 className="text-xs font-semibold text-gray-900">Create Task</h2>
              <button
            ...
            <form onSubmit={handleCreateTask} className="p-4 space-y-3">
              {createError && (
                <div className="rounded-md bg-red-50 p-2 text-[11px] text-red-600">{createError}</div>
              )}
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-medium text-gray-600 mb-1">Assignee</label>
                  <select
      ...
      {editTask && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh]">
          <div className="fixed inset-0 bg-black/40" onClick={() => setEditTask(null)} />
          <div className="relative z-10 w-full max-w-md rounded-md border border-gray-200 bg-white shadow-lg">
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
...
function TaskCard({ task, teamMembers, onStatusChange, onDelete, onEdit }) {
  ...
  return (
    <div className="rounded-md border border-gray-200 bg-white p-2.5 shadow-sm hover:shadow-sm transition-shadow group">
        ...
        <div className="relative flex-shrink-0">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-5 z-20 w-36 rounded-md border border-gray-200 bg-white py-1 shadow-md">
              <div className="px-2 py-1 text-[10px] font-medium text-gray-500 uppercase tracking-wider">Status</div>
              {STATUS_COLUMNS.map((col) => (
                <button
                  key={col.key}
                  ...
                  className={`w-full text-left px-2.5 py-1 text-[11px] hover:bg-gray-50 transition-colors cursor-pointer ${
                    String(task.status) === col.key ? "text-amber-600 font-medium" : "text-gray-600"
                  }`}
                >
                  {col.label}
                </button>
      ...
      {task.description && (
        <p className="mt-1 text-[10px] text-gray-500 line-clamp-2">{task.description}</p>
      )}
